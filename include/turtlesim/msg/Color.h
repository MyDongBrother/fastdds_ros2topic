// Copyright 2016 Proyectos y Sistemas de Mantenimiento SL (eProsima).
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

/*!
 * @file Color.h
 * This header file contains the declaration of the described types in the IDL file.
 *
 * This file was generated by the tool gen.
 */

#ifndef _FAST_DDS_GENERATED_TURTLESIM_MSG_COLOR_H_
#define _FAST_DDS_GENERATED_TURTLESIM_MSG_COLOR_H_


#include <stdint.h>
#include <array>
#include <string>
#include <vector>
#include <map>
#include <bitset>

#if defined(_WIN32)
#if defined(EPROSIMA_USER_DLL_EXPORT)
#define eProsima_user_DllExport __declspec( dllexport )
#else
#define eProsima_user_DllExport
#endif  // EPROSIMA_USER_DLL_EXPORT
#else
#define eProsima_user_DllExport
#endif  // _WIN32

#if defined(_WIN32)
#if defined(EPROSIMA_USER_DLL_EXPORT)
#if defined(Color_SOURCE)
#define Color_DllAPI __declspec( dllexport )
#else
#define Color_DllAPI __declspec( dllimport )
#endif // Color_SOURCE
#else
#define Color_DllAPI
#endif  // EPROSIMA_USER_DLL_EXPORT
#else
#define Color_DllAPI
#endif // _WIN32

namespace eprosima {
namespace fastcdr {
class Cdr;
} // namespace fastcdr
} // namespace eprosima


namespace turtlesim {
    namespace msg {
        /*!
         * @brief This class represents the structure Color defined by the user in the IDL file.
         * @ingroup COLOR
         */
        class Color
        {
        public:

            /*!
             * @brief Default constructor.
             */
            eProsima_user_DllExport Color();

            /*!
             * @brief Default destructor.
             */
            eProsima_user_DllExport ~Color();

            /*!
             * @brief Copy constructor.
             * @param x Reference to the object turtlesim::msg::Color that will be copied.
             */
            eProsima_user_DllExport Color(
                    const Color& x);

            /*!
             * @brief Move constructor.
             * @param x Reference to the object turtlesim::msg::Color that will be copied.
             */
            eProsima_user_DllExport Color(
                    Color&& x);

            /*!
             * @brief Copy assignment.
             * @param x Reference to the object turtlesim::msg::Color that will be copied.
             */
            eProsima_user_DllExport Color& operator =(
                    const Color& x);

            /*!
             * @brief Move assignment.
             * @param x Reference to the object turtlesim::msg::Color that will be copied.
             */
            eProsima_user_DllExport Color& operator =(
                    Color&& x);

            /*!
             * @brief Comparison operator.
             * @param x turtlesim::msg::Color object to compare.
             */
            eProsima_user_DllExport bool operator ==(
                    const Color& x) const;

            /*!
             * @brief Comparison operator.
             * @param x turtlesim::msg::Color object to compare.
             */
            eProsima_user_DllExport bool operator !=(
                    const Color& x) const;

            /*!
             * @brief This function sets a value in member r
             * @param _r New value for member r
             */
            eProsima_user_DllExport void r(
                    uint8_t _r);

            /*!
             * @brief This function returns the value of member r
             * @return Value of member r
             */
            eProsima_user_DllExport uint8_t r() const;

            /*!
             * @brief This function returns a reference to member r
             * @return Reference to member r
             */
            eProsima_user_DllExport uint8_t& r();

            /*!
             * @brief This function sets a value in member g
             * @param _g New value for member g
             */
            eProsima_user_DllExport void g(
                    uint8_t _g);

            /*!
             * @brief This function returns the value of member g
             * @return Value of member g
             */
            eProsima_user_DllExport uint8_t g() const;

            /*!
             * @brief This function returns a reference to member g
             * @return Reference to member g
             */
            eProsima_user_DllExport uint8_t& g();

            /*!
             * @brief This function sets a value in member b
             * @param _b New value for member b
             */
            eProsima_user_DllExport void b(
                    uint8_t _b);

            /*!
             * @brief This function returns the value of member b
             * @return Value of member b
             */
            eProsima_user_DllExport uint8_t b() const;

            /*!
             * @brief This function returns a reference to member b
             * @return Reference to member b
             */
            eProsima_user_DllExport uint8_t& b();


            /*!
             * @brief This function returns the maximum serialized size of an object
             * depending on the buffer alignment.
             * @param current_alignment Buffer alignment.
             * @return Maximum serialized size.
             */
            eProsima_user_DllExport static size_t getMaxCdrSerializedSize(
                    size_t current_alignment = 0);

            /*!
             * @brief This function returns the serialized size of a data depending on the buffer alignment.
             * @param data Data which is calculated its serialized size.
             * @param current_alignment Buffer alignment.
             * @return Serialized size.
             */
            eProsima_user_DllExport static size_t getCdrSerializedSize(
                    const turtlesim::msg::Color& data,
                    size_t current_alignment = 0);


            /*!
             * @brief This function serializes an object using CDR serialization.
             * @param cdr CDR serialization object.
             */
            eProsima_user_DllExport void serialize(
                    eprosima::fastcdr::Cdr& cdr) const;

            /*!
             * @brief This function deserializes an object using CDR serialization.
             * @param cdr CDR serialization object.
             */
            eProsima_user_DllExport void deserialize(
                    eprosima::fastcdr::Cdr& cdr);



            /*!
             * @brief This function returns the maximum serialized size of the Key of an object
             * depending on the buffer alignment.
             * @param current_alignment Buffer alignment.
             * @return Maximum serialized size.
             */
            eProsima_user_DllExport static size_t getKeyMaxCdrSerializedSize(
                    size_t current_alignment = 0);

            /*!
             * @brief This function tells you if the Key has been defined for this type
             */
            eProsima_user_DllExport static bool isKeyDefined();

            /*!
             * @brief This function serializes the key members of an object using CDR serialization.
             * @param cdr CDR serialization object.
             */
            eProsima_user_DllExport void serializeKey(
                    eprosima::fastcdr::Cdr& cdr) const;

        private:

            uint8_t m_r;
            uint8_t m_g;
            uint8_t m_b;
        };
    } // namespace msg
} // namespace turtlesim

#endif // _FAST_DDS_GENERATED_TURTLESIM_MSG_COLOR_H_